...
typedef struct {
	sender_window_t* wnd_send;//接收窗口
  	receiver_window_t* wnd_recv;//发送窗口
} window_t;
...
typedef struct
{
    sockQueue half_conn_socks;  // 半连接队列，收到SYN和SYN-ACK，缺少第三次握手的状态
    sockQueue fully_conn_socks; // 完全连接队列，已经完成三次握手
} TcpSocksQueue;
